...
function getscheduledetailid($id){
    $db = Database::instance();
    $db->select('scheduled_batch', array('id'=>$id),'' , 'id DESC','*');
...
function getschlist($curr){
    $db = Database::instance();
    $db->select('lead_scheduler',array('status'=>'pending','scheduleat'=>$curr), false, false,"AND","*");
    $result=$db->result_array();
    return $result;
...
function getscheduledetail($id){
    $db = Database::instance();
    $db->select('lead_scheduler',array('schedule_id'=>$id), false, false,'',"*");
    $result=$db->result_array();
    return $result[0];
