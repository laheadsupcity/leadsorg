...
//$db->select('scdleadtask', array('batchid' => $batchid), false, false,'','*');
$db->select('scheduled_lead_task', array('batchid' => $batchid,'status'=>'Active'), false, false,'AND','date_format(`starttask`, "%m/%d/%Y %h:%i %p") as date, taskname,period');
$result=$db->result_array();
//error_log("===========leardcount=============".print_r($result, true));
$count= count($result);
if($count==0){
	$value = array('msg' => 'Add');
